...
set_default_bindings(MiltonBindings* bs)
{
    ...
    binding(bs, Modifier_NONE, Binding::ESC, Action_ESCAPE);
    binding(bs, Modifier_NONE, 's', Action_SELECT);
...
void
binding_dispatch_action(BindableAction a, MiltonInput* out_input, Milton* milton, v2i pointer)
{
    ...
    switch (a) {
        ...
        case Action_REDO: {
            out_input->flags |= MiltonInputFlags_REDO;
        } break;
        case Action_UNDO: {
            out_input->flags |= MiltonInputFlags_UNDO;
        } break;
        case Action_EXPORT: {
            out_input->mode_to_set = MiltonMode::EXPORTING;
        } break;
        case Action_QUIT: {
        ...
        case Action_SAVE_AS: {
            ...
            if ( name ) {
                milton_log("Saving to %s\n", name);
                milton_set_canvas_file(milton, name);
                out_input->flags |= MiltonInputFlags_SAVE_FILE;
        ...
        case Action_OPEN: {
            ...
            if ( fname ) {
                ...
                out_input->flags |= MiltonInputFlags_OPEN_FILE;
            }
        } break;
        ...
        case Action_MODE_ERASER: {
            out_input->mode_to_set = MiltonMode::ERASER;
        } break;
        case Action_MODE_PEN: {
            out_input->mode_to_set = MiltonMode::PEN;
        } break;
        case Action_MODE_EYEDROPPER: {
            out_input->mode_to_set = MiltonMode::EYEDROPPER;
        } break;
        case Action_MODE_PRIMITIVE: {
            out_input->mode_to_set = MiltonMode::PRIMITIVE;
        } break;
        ...
        case Action_ESCAPE: {
            milton_escape(milton);
        } break;
        case Action_SELECT: {
            out_input->mode_to_set = MiltonMode::SELECT;
        } break;
